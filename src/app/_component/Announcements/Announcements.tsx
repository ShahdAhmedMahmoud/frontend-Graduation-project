...
import React, { useEffect, useState } from "react";
import { useSession } from "next-auth/react";
import { Bell, BookOpen, CheckCircle, AlertCircle, ExternalLink, Calendar, Loader2 } from "lucide-react";
import { useNotification } from "@/context/NotificationContext";
...
type Announcement = {
  ...
  title: string;
  content: string;
  type: "meeting" | "general" | "assignment" | "grades" | "missed_meeting";
  course: {
    _id: string;
    name: string;
    code: string;
...
const typeBadge: Record<string, string> = {
  ...
  grades: "bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-300",
  general: "bg-gray-100 text-gray-600 dark:bg-gray-900 dark:text-gray-300",
  missed_meeting: "bg-red-100 text-red-500 dark:bg-red-900 dark:text-red-300",
};
...
const typeIcon: Record<string, React.ReactNode> = {
  ...
  grades: <CheckCircle className="w-3.5 h-3.5" />,
  general: <AlertCircle className="w-3.5 h-3.5" />,
  missed_meeting: <AlertCircle className="w-3.5 h-3.5" />,
};
...
// Ensures the meeting link always has a protocol.
function normalizeMeetingUrl(url: string) {
  if (!url) return "";
...
export default function Announcements() {
  ...
  // Records attendance on the server first, then opens the meeting link.
  // Falls back to the announcement's own meetingUrl if the join call fails,
  // so the student is never blocked from joining.
  const handleJoinMeeting = async (meetingId: string, fallbackUrl: string) => {
    setJoining(meetingId);
    try {
      const res = await fetch(`${API_BASE_URL}/meetings/${meetingId}/join`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${session?.token}`,
          "Content-Type": "application/json",
      ...
      const urlToOpen = data?.data?.meetingUrl || fallbackUrl;
      window.open(normalizeMeetingUrl(urlToOpen), "_blank", "noopener,noreferrer");
    } catch {
      // Even if attendance recording fails, still let the student join.
      window.open(normalizeMeetingUrl(fallbackUrl), "_blank", "noopener,noreferrer");
    } finally {
      setJoining(null);
    }
  };

  // Show only the latest 4, newest first.
              ...
              <div key={item._id} className="py-4 first:pt-0 last:pb-0">
                ...
                {item.meeting && item.type === "meeting" && (
                  <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-3 mb-3">
                    ...
                    <button
                      onClick={() => handleJoinMeeting(item.meeting!._id, item.meeting!.meetingUrl)}
                      disabled={joining === item.meeting._id}
                      className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-blue-600 text-white text-xs font-semibold hover:bg-blue-700 transition disabled:opacity-60"
                    >
                      {joining === item.meeting._id ? (
